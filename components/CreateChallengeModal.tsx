...
export default function CreateChallengeModal({ open, onClose, onCreated }: {
  ...
  const updateAct = (i: number, f: Partial<Act>) =>
    setActs(prev => prev.map((a, idx) => idx === i ? { ...a, ...f } : a));
  ...
  const inputCls = 'w-full bg-white rounded-xl px-4 py-3 text-gray-900 text-sm placeholder-gray-400 outline-none focus:ring-2 focus:ring-[#FF6B6B]';
    ...
    <div className="fixed inset-0 bg-black/70 flex items-end justify-center z-50">
      <div className="w-full max-w-lg bg-gradient-to-b from-[#1a1a2e] to-[#16213e] rounded-t-3xl p-6 pb-12 max-h-[92vh] overflow-y-auto">
        ...
        <div className="flex justify-between items-center mb-5">
          ...
          <button onClick={onClose} className="text-white/50 text-3xl leading-none w-9 h-9 flex items-center justify-center">&times;</button>
        </div>
        ...
        <label className="block text-[10px] font-bold text-white/40 uppercase tracking-widest mb-2">Activiteiten</label>
